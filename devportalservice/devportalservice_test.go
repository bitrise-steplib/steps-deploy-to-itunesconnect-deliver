...
func TestGetAppleDeveloperConnection(t *testing.T) {
	tests := []struct {
		...
		{
			name: "API key-based Apple Developer Connection set for the build",
			response: &http.Response{
				StatusCode: 200,
		...
		{
			name: "Apple ID-based and API key-based Apple Developer Connection set for the build, test device available",
			response: &http.Response{
				StatusCode: 200,
...
func TestAppleIDConnectionFastlaneLoginSession(t *testing.T) {
	tests := []struct {
		name string
